# Тестовая сборка под чистым applicationId com.djiquicktest -> build/QuickToggleTest.apk.
# Отличие от build.ps1: манифест AndroidManifest.test.xml. Тот же src/res/assets и подпись.
...
$PROJ  = $PSScriptRoot
$BUILD = Join-Path $PROJ "build-test"
...
Write-Host "== aapt2 link (test manifest + res + assets) =="
$assetArgs = @()
if (Test-Path "$PROJ\assets") { $assetArgs = @("-A", "$PROJ\assets") }
& "$BT\aapt2.exe" link -I $AJAR --manifest "$PROJ\AndroidManifest.test.xml" "$BUILD\res.zip" @assetArgs `
...
& "$BT\apksigner.bat" sign --ks $KS --ks-pass pass:android --key-pass pass:android --ks-key-alias androiddebugkey --out "$BUILD\QuickToggleTest.apk" "$BUILD\app.aligned.apk"
if ($LASTEXITCODE -ne 0) { Die "apksigner" }

$sz = [math]::Round((Get-Item "$BUILD\QuickToggleTest.apk").Length/1KB,0)
Write-Host ("RESULT: " + (Join-Path $BUILD "QuickToggleTest.apk") + "  ($sz KB)") -ForegroundColor Green
